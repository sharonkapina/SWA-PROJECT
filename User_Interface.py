...
filename = "/Users/sharonkapina/Downloads/SWA_CODE/options.xlsx"
...
json_dir = "/Users/sharonkapina/Downloads/SWA_CODE/AUSTRALIA ANZSIC"
...
app_ui = ui.page_sidebar(
    ...
    if (el && label) {
        el.style.width = message.progress + "%";
        label.innerText = message.progress + "%";
    }
    });
    """)
)
...
def server(input, output, session):
    ...
    progress_visible = reactive.value(False)
    ...
    def progress_ui():
        if not progress_visible():
            return None
        print("🔥 Rendering progress_ui")
        return ui.tags.div([
            ui.tags.div("🔍 Scraping in progress...", style="font-weight: bold; color: red;"),
            ui.tags.div(class_="progress", style="height: 25px;", children=[
            ui.tags.div("0%", id="progress-label",
                class_="progress-bar progress-bar-striped progress-bar-animated",
                role="progressbar",
                id_="progress-bar-fill",
                style="height: 30px; background-color: #28a745; color: white;")
        ])
    ])
    ...
    def _():
        if input.submit():
            ...
            asyncio.create_task(urlscrapper.generate_urls(user_inputs, matched_orgs, session=session))
